...
def drawPixel(position, color):
    x = position[0]
    y = position[1]
    if(x>=I_WIDTH or x<0 or y<0 or y>=I_HEIGHT):
        return
    RGBS[y][x][0] = color[0]
    RGBS[y][x][1] = color[1]
    RGBS[y][x][2] = color[2]
...
def rasterizerLine(startf, endf):
    startf = np.matmul(startf, CAMERA_MATRIX)
    endf = np.matmul(endf, CAMERA_MATRIX)
...
def genCamMatrixByCamPos(position):
    z = norm(position)
    x = norm(np.cross((0,1,0), z)) # TODO: z=(0,1,0)
    y = norm(np.cross(z, x))
    return np.array([x,y,z]).transpose()

# drawLine((100,100), (800,300))
# rasterizerLine((1,1,1), (-1,1,1))
# rasterizerLine((1,1,1), (-1,-1,-1))
data = loadGltf("fish.gltf")
# print(data)
